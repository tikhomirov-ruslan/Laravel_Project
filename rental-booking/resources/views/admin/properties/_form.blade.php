<div class="grid gap-4">
    <div>
        <label class="text-sm font-medium text-slate-700" for="user_id">{{ __('ui.form.owner') }}</label>
    ...
    <div>
        <label class="text-sm font-medium text-slate-700" for="category_id">{{ __('ui.form.category') }}</label>
    ...
    <div>
        <label class="text-sm font-medium text-slate-700" for="title">{{ __('ui.form.title') }}</label>
        <input id="title" name="title" type="text" value="{{ old('title', $property->title) }}" class="mt-2 block w-full rounded-2xl border-slate-300">
    </div>

    <div>
        <label class="text-sm font-medium text-slate-700" for="address">{{ __('ui.form.address') }}</label>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div>
            <label class="text-sm font-medium text-slate-700" for="price_per_night">{{ __('ui.form.price_per_night') }}</label>
        ...
        <div>
            <label class="text-sm font-medium text-slate-700" for="max_guests">{{ __('ui.form.max_guests') }}</label>
    ...
    <div>
        <label class="text-sm font-medium text-slate-700" for="description">{{ __('ui.form.description') }}</label>
    ...
    <div>
        <label class="text-sm font-medium text-slate-700" for="images">{{ __('ui.form.property_photos') }}</label>
        ...
        <p class="mt-2 text-xs text-slate-500">{{ __('ui.form.photos_help') }}</p>
    </div>

    @if ($property->exists && $property->images->isNotEmpty())
        <div>
            <p class="text-sm font-medium text-slate-700">{{ __('ui.form.current_images') }}</p>
            <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($property->images as $image)
                    <label class="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                        <img src="{{ $image->url }}" alt="{{ __('ui.form.property_photo_alt') }}" class="h-36 w-full object-cover">
    ...
    <div>
        <p class="text-sm font-medium text-slate-700">{{ __('ui.form.amenities') }}</p>
