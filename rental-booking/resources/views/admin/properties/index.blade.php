<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">{{ __('ui.admin.properties_title') }}</h1>
                <p class="mt-1 text-sm text-slate-500">{{ __('ui.admin.properties_subtitle') }}</p>
            </div>
            <a href="{{ route('admin.properties.create') }}" class="rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white">
                {{ __('ui.admin.add_property') }}
            </a>
        </div>
    </x-slot>

    <div class="space-y-4">
        @foreach ($properties as $property)
            <div class="rounded-3xl bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div class="flex gap-4">
                        <img src="{{ $property->primary_image_url }}" alt="{{ $property->title }}" class="h-28 w-40 rounded-2xl object-cover">
                        <div>
                            <p class="text-xs uppercase tracking-wide text-orange-600">{{ $property->category?->name }}</p>
                            <h2 class="mt-2 text-xl font-semibold text-slate-900">{{ $property->title }}</h2>
                            <p class="mt-2 text-sm text-slate-600">{{ $property->address }}</p>
                            <p class="mt-2 text-sm text-slate-500">{{ __('ui.admin.owner', ['name' => $property->owner?->name]) }}</p>
                            <p class="mt-2 text-sm text-slate-500">{{ __('ui.admin.images', ['count' => $property->images->count()]) }}</p>
                        </div>
                    </div>

                    <div class="text-left md:text-right">
                        <p class="text-lg font-bold text-slate-900">{{ number_format((float) $property->price_per_night, 0, '.', ' ') }} KZT</p>
                        <p class="mt-1 text-sm text-slate-500">{{ __('ui.admin.reviews_count', ['count' => $property->reviews->count()]) }}</p>
                    </div>
                </div>

                <div class="mt-5 flex flex-wrap gap-3 border-t border-slate-200 pt-5">
                    <a href="{{ route('admin.properties.edit', $property) }}" class="rounded-full border border-slate-300 px-4 py-2 text-sm text-slate-700">
                        {{ __('ui.admin.edit') }}
                    </a>
                    <form method="POST" action="{{ route('admin.properties.destroy', $property) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="rounded-full border border-rose-300 px-4 py-2 text-sm text-rose-700">
                            {{ __('ui.admin.delete') }}
                        </button>
                    </form>
                </div>
            </div>
        @endforeach
    </div>
</x-app-layout>
